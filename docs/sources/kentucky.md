...
`npm run kentucky:candidates:audit` writes `public/results/kentucky-2022-state-candidate-readiness.json`. It distinguishes districts with recap-derived candidate metadata from districts that still require certified-PDF header extraction; reconciled vote totals are never promoted with guessed candidate names.
