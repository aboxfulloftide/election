...
npm run kentucky:certified:merge
...
The certified PDF is image-only. Run `kentucky:certified:ocr` on a machine with `ocrmypdf` and `pdftotext` installed; it creates an ignored searchable PDF and layout-preserving text file. Then `kentucky:certified:parse` emits diagnostic U.S. Senate county rows, all six U.S. House county rows and totals, and printed totals for the 19 State Senate and 100 State House sections under the ignored raw-data directory. Verified OCR corrections reconcile all six House districts and all 119 state legislative districts. `kentucky:certified:merge` replaces the partial 2022 House contests in the published summary after requiring all six districts to reconcile; candidate-name extraction is still required before the 119 state legislative contests can be promoted.

`npm run kentucky:candidates:audit` writes `public/results/kentucky-2022-state-candidate-readiness.json`. It validates certified-PDF header extraction for all 19 State Senate and 100 State House districts. `npm run kentucky:certified:state:merge` promotes those 119 reconciled contests; Kentucky remains `partial` until the other years and the Senate write-in lane are complete.
