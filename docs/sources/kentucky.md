...
The certified PDF is image-only. Run `kentucky:certified:ocr` on a machine with `ocrmypdf` and `pdftotext` installed; it creates an ignored searchable PDF and layout-preserving text file. Then `kentucky:certified:parse` emits diagnostic U.S. Senate county rows, all six U.S. House county rows and totals, and printed totals for the 19 State Senate and 100 State House sections under the ignored raw-data directory. Verified OCR corrections reconcile all six House districts. `kentucky:certified:merge` replaces the partial 2022 House contests in the published summary after requiring all six districts to reconcile; the Senate and state legislative contests remain separate validation work.
