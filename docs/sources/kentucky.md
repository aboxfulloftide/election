...
The certified PDF is image-only. Run `kentucky:certified:ocr` on a machine with `ocrmypdf` and `pdftotext` installed; it creates an ignored searchable PDF and layout-preserving text file. Then `kentucky:certified:parse` emits diagnostic U.S. Senate county rows, all six U.S. House county rows and totals, and printed totals for the 19 State Senate and 100 State House sections under the ignored raw-data directory. The Senate Republican/Democratic columns reconcile. House district rows are now grouped and checked against printed totals; OCR discrepancies remain explicitly flagged, so no new contest values are published until independently resolved.
