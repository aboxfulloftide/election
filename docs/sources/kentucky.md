...
The certified PDF is image-only. Run `kentucky:certified:ocr` on a machine with `ocrmypdf` and `pdftotext` installed; it creates an ignored searchable PDF and layout-preserving text file. Then `kentucky:certified:parse` emits diagnostic U.S. Senate county rows, all six U.S. House totals, and printed totals for the State Senate and State House sections under the ignored raw-data directory. The Senate Republican/Democratic columns currently reconcile; write-in columns and county-level rows still require validation before any values can replace the partial recap-derived summary.
