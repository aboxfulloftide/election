...
The certified PDF is image-only. Run `kentucky:certified:ocr` on a machine with `ocrmypdf` and `pdftotext` installed; it creates an ignored searchable PDF and layout-preserving text file. Then `kentucky:certified:parse` emits diagnostic U.S. Senate county rows and printed totals for all six U.S. House districts under the ignored raw-data directory. The Senate Republican/Democratic columns currently reconcile; write-in columns and county-level House rows still require validation before any values can replace the partial recap-derived summary.
